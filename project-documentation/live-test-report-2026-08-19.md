...
اختُبرت **replica-01 وreplica-02 فقط** عبر HTTP API المباشر. لم تُلمس replica-04. نُفذت الحالات بالتسلسل لكل نسخة: طلب نص واحد، طلب بحث حي واحد، ثم **طلب صورة واحد فقط** لتقليل استهلاك حصة ChatGPT. بدأ الفحص بعد تأكيد أن `/health` أعاد HTTP 200 و`ready=true` للنسختين.
