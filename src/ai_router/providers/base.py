    ...
    def visit(node: Any, *, citation_field: bool = False) -> None:
        if isinstance(node, dict):
            for key, child in node.items():
                visit(child, citation_field=citation_field or str(key).lower() in {"url", "uri", "source_url", "link", "href", "source", "canonical_url", "website", "citation", "citations", "annotation", "annotations", "content", "tool_output", "search_result", "grounding_chunk"})
