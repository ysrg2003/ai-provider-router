...
class RouterStore:
    ...
    def get_state(self, provider: str, model: str, key_id: str) -> dict[str, Any] | None:
        with self._connect() as connection:
            row = connection.execute(
                "SELECT provider, model, key_id, project, consecutive_failures, total_calls, total_successes, cooldown_until, last_error, updated_at FROM provider_state WHERE provider = ? AND model = ? AND key_id = ?",
                (provider, model, key_id),
            ).fetchone()
        return dict(row) if row else None

    def is_cooling(self, provider: str, model: str, key_id: str) -> bool:
        state = self.get_state(provider, model, key_id)
    ...
    def stats(self) -> dict[str, int]:
        ...
        return {"calls": calls, "provider_states": states}
