...
class ChatGPTImageAdapterTests(unittest.TestCase):
    def test_job_poll_and_image_download(self):
        ...
        with patch(
            "ai_router.providers.chatgpt_image.requests.post",
            return_value=FakeResponse(200, {"job_id": "job-1", "status": "queued"}),
        ) as post, patch(
            "ai_router.providers.chatgpt_image.requests.get",
            side_effect=[
    ...
    def test_auth_error_is_not_retryable(self):
        ...
        with patch(
            "ai_router.providers.chatgpt_image.requests.post",
            return_value=FakeResponse(401, {"error": {"message": "Invalid API Key"}}),
