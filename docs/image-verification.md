...
أعيد بناء خدمة `chatgpt-api` من ملف ZIP الأصلي `chatgpt-without-api-main_2.zip`، الذي يفتح المحادثة العادية في `https://chatgpt.com/`. أضيفت cookies بصيغة Netscape من خلال Secret، وأصبحت آلية `/v1/jobs` مع polling هي النقل المعتمد للنص والبحث والصور؛ الاختلاف فقط في استخراج النص أو `image_url` من النتيجة المكتملة. داخل الخدمة يفتح job الصورة direct conversation عادية في Playwright.
...
يستخدم النظام `/v1/jobs` مع polling للنص والبحث والصورة. أصلحت SQLite persistence مشكلة `Job not found` عبر العمليات، ولذلك يستطيع job الصورة تشغيل نفس المحادثة العادية وPlaywright دون إبقاء HTTP request متزامنًا مفتوحًا عبر HF ingress. أثبت الاختبار الحي أن direct `/v1/chat/completions` قد يعيد HTTP 502 بعد انتظار قصير، لذلك لا يعد هو النقل المعتمد للراوتر. اكتُشف أن ChatGPT في واجهة الويب لا يلزم أن يعيد `image_url` داخل JSON؛ الصورة الأصلية تظهر كـ`<img>` جديد داخل `main`، وقد يكون `src` من `estuary/content` أو `files/download` أو `oaiusercontent` أو `data:`/`blob`. لذلك عُدّل `chatgpt-api` لالتقاط asset جديد من DOM بعد baseline، خارج assistant bubble، ثم تنزيل bytes من المتصفح وتطبيعها إلى `image_url` داخليًا. وإذا ظهر رد نصي أول بلا صورة، تنفذ الخدمة متابعة واحدة داخل نفس الصفحة والمحادثة: `Please generate the image from my previous request now. Image only.` ثم تعيد فحص DOM؛ فلا تُعتبر رسالة quota الأولى نهاية نهائية قبل اكتمال دورة الطلب الثاني. وعُدّل الراوتر لقبول `image_url` و`output_image` و`image_generation_call` و`b64_json` وأشكال asset المتداخلة.
...
بعد تجدد قدرة إنشاء الصور، شغّل **Actions → Live smoke → Run workflow → `chatgpt_conversation_image`**. هذا الاختبار يستخدم `/v1/jobs` ثم polling للصورة، وينتظر الطلب الأول ثم متابعة واحدة داخل نفس المحادثة قبل الحكم النهائي؛ لا تستخدم `image` وحده لإثبات ChatGPT، لأنه يسمح بالـfallback. يعتبر التحقق ناجحًا فقط عندما يعيد التقرير `status: passed` و`provider: chatgpt_conversation` و`mime_type` للصورة و`bytes_base64` أكبر من صفر. إذا أعاد ChatGPT رسالة Free plan limit، انتظر reset ولا تغيّر parser بناءً على تلك النتيجة.
