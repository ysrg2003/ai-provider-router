...
قد يستغرق توليد الصورة عدة دقائق، ولذلك يحدد provider مهلة 300 ثانية ويستخدم adapter مهلة لا تقل عن 240 ثانية. إذا لم تعُد `images`، يسجل الراوتر فشل المصدر الأول وينتقل إلى المصدر التالي وفق سياسة fallback؛ لا ينبغي اعتبار نص الوصف وحده صورة ناجحة.
...
|---|---|
| `compileall` | ناجح |
| اختبارات الوحدة الكاملة | ناجحة، 34 اختبارًا |
...
| اختبار حي للصورة | لم يُعد Space `images` في المحاولة الحالية؛ لذلك رفض adapter اعتبارها صورة ناجحة وسجل fallback-ready failure |

النتيجة الأخيرة تعني أن wiring الراوتر والـadapter صحيحان، لكنها لا تثبت توفر توليد الصور في جلسة Space الحالية. يجب إعادة اختبار الصورة بعد فحص جلسة ChatGPT داخل Space أو تحديث استخراج DOM، بدل إخفاء الفشل أو تحويل نص عادي إلى ملف صورة.
